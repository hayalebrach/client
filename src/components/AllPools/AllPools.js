...
export default  function AllPools(){
  ...
  return(
    ...
    <ul>
        {
           Pools.map(pool=> <><div  className="pool"><div>  <img src={`Pic/${pool.Pic}.jpg`} className="img"/>  </div> <br/><b>{pool.Name}</b><br/>{pool.Adress}<br/> <br/>{pool.Price}<input type="button" className="button1" value="לפרטים" onClick={()=>Func(pool)}></input></div></> )
        }
    </ul>
